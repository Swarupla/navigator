...
$(document).ready(function () {
  ...
  $('.tab-change').click(function () {
    $('.no-click-change .inner-tab-click li').removeClass('active');
    $('.no-click-change .tab-content-wrapper .tab-pane').removeClass('active in');
    $('.no-click-change .inner-tab-click li:nth-child(3)').addClass('active');
    $('.no-click-change .tab-content-wrapper .tab-pane:nth-child(1)').addClass('active in');
  });
  ...
  const counter_section1 = document.querySelector(".flip-wrapper:nth-child(1) .number-per");
  const counter_section2 = document.querySelector(".flip-wrapper:nth-child(2) .number-per");
  const counter_section3 = document.querySelector(".flip-wrapper:nth-child(3) .number-per");
  const counter_section4 = document.querySelector(".flip-wrapper:nth-child(4) .number-per");
  const counter_section5 = document.querySelector(".flip-wrapper:nth-child(5) .number-per");
  ...
  $(".navigation-control.left").click(function () {
    ...
    // currentIndexLeft++; //1
    $('.onboardingcontent').removeClass('animactive');

    setTimeout(() => {
      $('.onboardingcontent').addClass('animactive');
    }, 500);
  ...
  let intervalId;
  let datascrentwo = 'mainscreen2_4';
  // main box navigation 
  $('.block-wrapper .mainflexclick').click(function () {
    currentIndexLeft = 0;
    currentIndexRight = 0;
    let secondscreenid = $(this).attr('data-sec');
    ...
    if (blockatr == 'click1') {
    ...
    else if (secondscreenid == 2) {
      $('.child-screenwrapper').removeClass('active');
      $('.child-screenwrapper[data-val=' + datascrentwo + ']').addClass('active');
      currentIndexRight++;
      currentIndexRight++;
      currentIndexRight++;
      currentIndexLeft++;
      currentIndexLeft++;
      currentIndexLeft++;
      $(".content-nav p").html('Digital Capabilities');
    }
    else if (secondscreenid == 1) {
      currentIndexRight = 0;
      currentIndexLeft = 0;
    }
  ...
  $('.screen1-innerclick').click(function () {
    ...
    else if (screen_inner == 'screen1_inner2') {
      $(".content-nav p").html('Banking Market Scan');
      $('.impactgenaitabs').removeClass('active');
    }
    else if (screen_inner == 'screen1_inner3') {
      $(".content-nav p").html('Mortage Value Chain');
      $('.impactgenaitabs').removeClass('active');
    }
    else {
      $('.impactgenaitabs').removeClass('active');
  ...
  $('.parent-wrapper header .back-button').click(function () {
    ...
    $(".homescreen-wrapper").addClass("active");
    $('.impactgenaitabs').addClass('active');
    $('.screen-wrapper').removeClass('active');
  ...
  $('.tab-new-click').click(function () {
    ...
    $('.change-content .internal-tabs .screen1-inner').removeClass('active');
    $('.change-content .internal-tabs .screen1-inner:nth-child(1)').addClass('active');
    $('.change-content .inner-header-tabs span').removeClass('active');
    $('.change-content .inner-header-tabs span:nth-child(1)').addClass('active');
  });
  $('.inner-clickthrough').click(function () {
    $('.linkedcontent').removeClass('active')
    $('.main-containers-wrapper').removeClass('active-container');
    let atrclick = $(this).attr('data-attr');
    let tabClick = $(this).attr('data-value');
    $('.back-button').attr('data-nav', atrclick);
    $('.screen-wrapper').removeClass('active');
    $('.child-screenwrapper').removeClass('active')
    $('.child-screenwrapper[data-val=' + atrclick + ']').addClass('active');
    // $('.child-screenwrapper[data-val=' + atrclick + ']').parent().addClass('active');
    $('.mainflexclick[data-value=' + tabClick + ']').trigger('click');
    // setTimeout(() => {
  ...
  $(".task-skillprofile-click").on("click", function () {
    if ($('.barssld').hasClass("active")) {
      $('.tasktext').addClass("active");
      $('.skilltext').removeClass("active");
    }
    else {
      $('.skilltext').addClass("active");
      $('.tasktext').removeClass("active");
    }
  });
  // end 
  ...
  $("#checkrollout").on("change", function () {
    if (this.checked) {
      $('.chartgraph').show();
      $('.barssld').hide();
      $('.switchtxt2').addClass('coloractivecls');
      $('.switchtxt').removeClass('coloractivecls');
    } else {
      $('.barssld').show();
      $('.chartgraph').hide();
      $('.switchtxt').addClass('coloractivecls');
      $('.switchtxt2').removeClass('coloractivecls');
    }
  });
